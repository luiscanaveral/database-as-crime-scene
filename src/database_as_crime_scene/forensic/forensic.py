...
class Forensic:
    ...
    def check_indexes(self):
        ...
        def _get_classification(row):
            ...
            pct_vs_total = float(row['pct_vs_total']) if row['pct_vs_total'] is not None else 0
        ...
        def _get_score(row):
            idx_scan = row['idx_scan']
            pct_vs_heap = float(row['pct_vs_heap']) if row['pct_vs_heap'] is not None else 0
            pct_vs_total = float(row['pct_vs_total']) if row['pct_vs_total'] is not None else 0
            is_pk = bool(row['is_pk']) if 'is_pk' in row else False
            is_unique = bool(row['is_unique']) if 'is_unique' in row else False
            return self.compute_score(pct_vs_heap, pct_vs_total, idx_scan, is_pk, is_unique)
        ...
        df_merged = pd.merge(df_sorted, df_selectivity, on=['table_name', 'index_name'], how='left')

        log(df_merged, type="table")
