...
def add_friends(
    ...
    with engine.begin() as conn:
        for i in range(0, len(friendships), batch_size):
            ...
            values = ", ".join([
                f"({user_id}, {friend_id})" 
                for user_id, friend_id in batch
            ])
...
def generate_bulk_users(
    ...
    with engine.begin() as conn:
        ...
        result = conn.execute(
            query,
            {"start_id": start_id, "end_id": start_id + count - 1}
        )
...
def generate_bulk_posts_for_users(
    ...
    with engine.begin() as conn:
        ...
        result = conn.execute(
            query,
            {
                "min_posts": posts_per_user_min,
                "max_posts": posts_per_user_max
            }
        )
...
def generate_bulk_comments_for_posts(
    ...
    with engine.begin() as conn:
        ...
        result = conn.execute(
            query,
            {"max_comments": comments_per_post_max + 1}
        )
